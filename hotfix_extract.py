...
def _resolve_agent_member_path(
    agent_dest: Path, relative_parts: tuple[str, ...]
) -> Path | None:
    if not relative_parts:
        return None
    if any(part in {"", ".", ".."} for part in relative_parts):
        raise ValueError(f"unsafe agent archive path: {'/'.join(relative_parts)}")

    agent_root = agent_dest.resolve()
    target_path = agent_root.joinpath(*relative_parts).resolve()
    if target_path != agent_root and agent_root not in target_path.parents:
        raise ValueError(f"agent archive path escapes destination: {target_path}")
    return target_path
...
def _write_agent_member(
    ...
    target_path = _resolve_agent_member_path(agent_dest, relative_parts)
    ...
    try:
        with zipfile.ZipFile(archive, "r", metadata_encoding="utf-8") as zf:
            ...
            for member, relative_parts in agent_members:
                if member.endswith("/"):
                    target_path = _resolve_agent_member_path(
                        agent_dest, relative_parts
                    )
        ...
        with tarfile.open(archive, "r:*") as tf:
            ...
            for member, relative_parts in agent_members:
                ...
                if not member.isfile():
                    logger.warning("[热更新] 跳过非普通 agent 文件: %s", member.name)
                    continue
