...
- option `record<string, object>`  
  ...
  - type `string`  
    ...
    - `"show"`: 只读展示框，动态加载并展示文件内容（支持 Markdown/JSON）
  ...
  - shows `object[]`
    仅在 `type` 为 `"show"` 时使用。用于按顺序定义要展示的数据节点列表。
    ...
    - path `string`
      节点内容来源。可填写相对路径（相对于 interface.json 所在目录）或直接文本。支持国际化（以`$`开头）。
    - mode `"auto" | "md" | "json"`
      展示模式，默认 `"auto"`。
    - shows `object[]`
      子节点列表，支持递归嵌套。
    ...
    - 按 `shows` 中定义顺序展示节点内容。
    - 每个节点提供展开/收起交互。
    - 当 `mode=auto` 且来源为文件时：`.md` 按 Markdown 渲染，`.json/.jsonc` 以只读 JSON 卡片展示。
